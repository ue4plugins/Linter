...
FText FTooltipStringHelper::ParseFunctionRawTooltipGetDescription(FString RawTooltip, bool bRemoveNewlines/* = false*/) {
    ...
    for (FString Line : Lines) {
        ...
        if (Line.StartsWith(TEXT("@return"))) //@return is assumed to always be last
        {
            break;
        }
...
bool FTooltipStringHelper::FindAndUpdateArgumentTooltip(FText ArgumentName, FText Tooltip, TArray<TSharedPtr<FBPFunctionArgumentDescription>>& Arguments) {
    const TSharedPtr<FBPFunctionArgumentDescription>* FuncArg = Arguments.FindByPredicate([&](TSharedPtr<FBPFunctionArgumentDescription> Arg) {
        return Arg->ArgumentName.EqualTo(ArgumentName, ETextComparisonLevel::Quinary);
    });
    if (FuncArg != nullptr) {
